...
import gc  # Add missing gc import for memory management
from flask import Flask, request, jsonify, session, send_file
from flask_socketio import SocketIO
from flask_cors import CORS
from werkzeug.utils import secure_filename
# Import audio processing with fallback
try:
    from modules.audio_processing import extract_audio, filter_silence
except ImportError:
    logger = logging.getLogger(__name__)
    logger.error("Failed to import audio_processing module. Make sure pydub is installed.")
    
    # Define fallback functions
    def extract_audio(video_path, audio_path):
        raise ImportError("audio_processing module not available. Please install pydub.")
        
    def filter_silence(audio_path, output_path):
        raise ImportError("audio_processing module not available. Please install pydub.")
...
# Try importing sentence-transformers with fallback mechanism
try:
    ...
    logger = logging.getLogger(__name__)
    logger.warning("sentence-transformers not available. Chat functionality will use fallback methods.")
...
def get_sentence_model():
    """Lazy load the sentence transformer model with better error handling"""
    global _sentence_model
    
    # Check if sentence transformer is available
    if not SENTENCE_TRANSFORMER_AVAILABLE:
        logger.warning("Sentence transformer not available, using fallback method")
        return None
...
def generate_chat_response(user_message, transcription, history):
    ...
    try:
        ...
        logger.info(f"Processing semantic search query: '{enhanced_query}'")
...
def scheduled_cleanup():
    """Clean up old files and sessions with improved resource management"""
    try:
        ...
        logger.info(f"Cleanup completed: removed {files_removed} files, cleaned {sessions_cleaned} sessions")
...
# Original imports and setup code...
...
if __name__ == "__main__":
    try:
        ...
        # Make sure to clean up resources properly on exit
